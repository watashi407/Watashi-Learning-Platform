      ...
      <section className="space-y-8">
        <WorkspacePanel className="overflow-hidden p-0">
          <div className="relative min-h-[21rem]">
            ...
            <div className="relative flex h-full min-h-[21rem] flex-col justify-end p-8 text-white">
              ...
              <h1 className="mt-5 max-w-[14ch] font-display text-[clamp(2.3rem,5vw,4rem)] font-black leading-[0.92] tracking-[-0.07em]">
        ...
        <div>
          <div className="mb-5 flex items-center justify-between gap-4">
            <div>
              <h2 className="font-display text-[2rem] font-black tracking-[-0.05em] text-slate-950">Live Discussions</h2>
              <p className="mt-1 text-sm text-slate-500">Trending conversations with explicit module boundaries and cleaner feed composition.</p>
            </div>
            <div className="inline-flex rounded-full bg-[var(--color-watashi-surface-low)] p-1 text-[11px] font-bold">
              <span className="rounded-full bg-white px-3 py-1.5 text-[var(--color-watashi-emerald)] shadow-sm">Trending</span>
              <span className="px-3 py-1.5 text-slate-500">Latest</span>
            </div>
          </div>

          <div className="grid gap-5 md:grid-cols-2">
            {discussions.map((discussion, index) => (
              <WorkspacePanel key={discussion.title} className={cx('rounded-[2rem] p-6', index === 1 && 'bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(242,244,246,0.92))]')}>
                <WorkspaceEyebrow>{discussion.topic}</WorkspaceEyebrow>
                <h3 className="mt-4 font-display text-[1.65rem] font-black leading-tight tracking-[-0.05em] text-slate-950">
                  {discussion.title}
                </h3>
                <p className="mt-4 text-sm leading-7 text-slate-500">{discussion.body}</p>
      ...
      <aside className="space-y-6">
        <WorkspacePanel className="bg-[linear-gradient(180deg,rgba(255,255,255,0.98),rgba(242,244,246,0.94))]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-[1.55rem] font-black tracking-[-0.04em] text-slate-950">Community Voice</h2>
              <WorkspaceEyebrow>Impact tracker</WorkspaceEyebrow>
            </div>
          </div>
          <div className="mt-6 space-y-4">
            {communityStats.map((item) => (
              <div key={item.label} className="flex items-center justify-between rounded-[1.5rem] bg-white px-4 py-4 ring-1 ring-slate-100">
                <div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-1">
          ...
          <WorkspacePanel>
            ...
            <p className="mt-3 font-display text-[1.6rem] font-black tracking-[-0.05em] text-slate-950">Top Curators</p>
            <ol className="mt-5 space-y-3 text-sm text-slate-500">
